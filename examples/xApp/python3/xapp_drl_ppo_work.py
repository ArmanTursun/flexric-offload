...
ppo_agent = PPO(state_dim=2, action_dim=1, lr_actor=0.0001, lr_critic=0.0003,
...
class StateNormalizer:
    def __init__(self):
        ...
        self.bler_min = float('inf')
        self.bler_max = float('-inf')

    def update_bler(self, bler_mean, bler_max, bler_min):
        # Update the min and max for bler values dynamically
        self.bler_min = min(self.bler_min, bler_min)
        self.bler_max = max(self.bler_max, bler_max)
    ...
    def normalize_bler(self, value):
        # Min-max normalization for bler stats
        if self.bler_max == self.bler_min:
            return 0  # Avoid division by zero
        return (value - self.bler_min) / (self.bler_max - self.bler_min)
    ...
    def normalize_state(self, bler_mean, bler_max, bler_min, bler_skewness,
        ...
        # Update bler min/max before normalization
        self.update_bler(bler_mean, bler_max, bler_min)
        
        # Normalize the BLER stats
        normalized_bler_mean = self.normalize_bler(bler_mean)
        normalized_bler_max = self.normalize_bler(bler_max)
        normalized_bler_min = self.normalize_bler(bler_min)
...
def calculate_reward(current_bler, current_energy, previous_bler, previous_energy):
    ...
    if current_bler > bler_threshold:
        penalty = penalty_factor * current_bler #(current_bler - bler_threshold)
    else:
...
#def run_drl():
    ...
    max_ep_len = 10  # or any length you prefer
    update_timestep = max_ep_len
    action_std_decay_rate = 0.05
    min_action_std = 0.1
    action_std_decay_freq = update_timestep * 2
    ...
    while time_step <= max_training_timesteps:
        ...
        current_ep_reward = 0
        current_bler, current_energy = current_bler[0], current_energy[0]
        for t in range(1, max_ep_len + 1):
            ...
            reward = calculate_reward(next_bler[0], next_energy[0], current_bler, current_energy)
            ...
            if time_step % log_freq == 0:
                ...
                log_f.write('{},{},{},{},{},{},{},{},{},{}\n'.format(i_episode, time_step, float(action[0]), round(avg_reward, 4), current_bler, current_energy, next_bler[0], next_energy[0], update_latency, time.time_ns() / 1000000.0 - start_time))
                log_f.flush()
            ...
            current_bler, current_energy = next_bler[0], next_energy[0]
