...
class Critic(nn.Module):
    def __init__(self, state_size, action_size, hidden_units=(128, 128), learning_rate=0.001, tau=0.001):
        """
        Constructor for the Critic network in PyTorch

        Args:
        - state_size (int): Dimension of the input state (e.g., 8 for BLER and energy).
        - action_size (int): Dimension of the input action (e.g., 2 for weights).
        - hidden_units (tuple): Number of hidden units in each layer. Default: (128, 128).
        - learning_rate (float): Learning rate for training the model. Default: 0.001.
        ...
        # Combined pathway (after concatenation of state and action)
        self.fc2 = nn.Linear(hidden_units[0] + hidden_units[0], hidden_units[1])  # Double size due to concatenation
    ...
    def init_weights(self):
        ...
        for layer in [self.fc1_state, self.fc1_action, self.fc2, self.fc3]:
    ...
    def forward(self, state, action):
        ...
        combined = torch.cat([state_out, action_out], dim=-1)  # Concatenate along the last dimension
...
if __name__ == "__main__":
    ...
    action_tensor = torch.FloatTensor([[0.45, 0.55]])  # Example action (weights), shape (1, 2)
